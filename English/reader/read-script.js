...
async function loadDictionary(dictionaryPath) {
    try {
        ...
        if (!dictionaryPath) {
            console.log('No dictionary path provided, using empty dictionary');
            return;
        }
        ...
        console.log(`Loaded dictionary with ${Object.keys(dictionary).length} words from ${dictionaryPath}`);
        
    } catch (error) {
        console.error('Error loading dictionary:', error);
        // Keep empty dictionary if loading fails
    }
}
...
async function loadStory() {
    try {
        ...
        if (typeof window.storiesData !== 'undefined') {
            ...
            if (currentStory) {
                // Load the dictionary for this story
                await loadDictionary(currentStory.dictionary);
        ...
        // Load from database/main.js (go up one level from reader folder)
        ...
        // Try to load dictionary if it exists in fallback story
        if (currentStory.dictionary) {
            await loadDictionary(currentStory.dictionary);
        }
        displayStory(currentStory);
...
function getFallbackStory(storyId) {
    const fallbackStories = {
        1: {
            ...
            level: "beginner",
            wordCount: 350,
            dictionary: "../Dictionarys/main.json", // Updated path for your structure
            content: [
